...
import Protolude hiding (log, msg)

import Control.Monad.Catch (MonadMask)
import Control.Monad.Log (Handler, LoggingT, MonadLog, Severity(..), WithTimestamp(..), WithSeverity(..), BatchingOptions(..), logMessage, mapLogMessageM, renderWithSeverity, renderWithTimestamp, runLoggingT, timestamp, withFDHandler)
...
batchingOptions = BatchingOptions {flushMaxDelay = 0, flushMaxQueueSize = 1, blockWhenFull = True}

-- | Take a bunch of logs with severity and print them to stdout with timestamps.
withLogging :: (MonadMask m, MonadIO m, Pretty msg) => Severity -> LogM msg m a -> m a
withLogging severityThreshold body =
  withFDHandler batchingOptions stdout 0.4 80 $
  \stdoutHandler ->
     runLoggingT
       (withTimestamps body)
       (printLogs severityThreshold stdoutHandler)

withTimestamps :: (MonadIO m, MonadLog (WithTimestamp msg) m) => LoggingT msg m a -> m a
...
printLogs :: (Pretty a, MonadIO m) => Severity -> Handler m Doc -> WithTimestamp (WithSeverity a) -> m ()
